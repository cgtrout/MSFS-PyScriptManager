...
class OrderedLogger:
    ...
    def __init__(self, filename: str, level: int = logging.DEBUG, log_format: str | None = None) -> None:
        self.log_queue: PriorityQueue[tuple[float, int, str, str, tuple[Any, ...], dict[str, Any]]] = PriorityQueue()
        self.logger: logging.Logger = logging.getLogger("OrderedLogger")
        ...
        self.processor_thread: threading.Thread = threading.Thread(target=self._process_logs, daemon=True)
    ...
    def _emit_log(self, level: str, message: str, args: tuple[Any, ...], kwargs: dict[str, Any]) -> None:
    ...
    def log(self, level: str, message: str, *args: Any, **kwargs: Any) -> None:
        ...
        sequence_number: int = self._get_sequence_number()
